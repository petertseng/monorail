...
impl Board {
    ...
    fn makes_board_unsolvable(&self, c: Coordinate) -> bool {
        (self.occupied(Coordinate{row: 3, col: 0}) && c == Coordinate{row: 2, col: 1}) ||
        (self.occupied(Coordinate{row: 3, col: 0}) && c == Coordinate{row: 1, col: 1}) ||
        (self.occupied(Coordinate{row: 2, col: 0}) && c == Coordinate{row: 1, col: 1}) ||
        (self.occupied(Coordinate{row: 2, col: 1}) && c == Coordinate{row: 3, col: 0}) ||
        (self.occupied(Coordinate{row: 1, col: 1}) && c == Coordinate{row: 2, col: 0}) ||
        (self.occupied(Coordinate{row: 1, col: 1}) && c == Coordinate{row: 3, col: 0})
    }
    ...
    fn frontier(&self) -> Vec<Coordinate> {
        ...
        for row in 0..NUM_ROWS {
            for col in 0..NUM_COLS {
                ...
                if self.occupied(coord) || self.makes_board_unsolvable(coord) {
                    continue;
                }
        ...
        for frontier_space in self.frontier().iter() {
            for move_type in POSSIBLE_MOVE_TYPES.iter() {
                ...
                for other_space in mov.coords().iter() {
                    if self.occupied(*other_space) || self.makes_board_unsolvable(*other_space) {
